...
class Request
{
    ...
    private function _apiError($status_code, $responseHeader, $responseBody, $responseJSON)
    {
        ...
        if ($status_code === 429) {
            throw new RateLimitError($message, $status_code, $responseHeader, $responseBody,
                $this->_parseHeader($responseHeader), $responseJSON, $incorrectParameter);
        }

        throw new RequestError($message, $status_code, $responseHeader, $responseBody,
            $this->_parseHeader($responseHeader), $responseJSON, $incorrectParameter);
    }
